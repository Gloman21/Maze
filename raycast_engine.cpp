...
std::vector<std::vector<int> > generateMap(int width, int height) {
    ...
    // Filling the inside with random walls and empty spaces
...
bool initialize_SDL(SDL_Instance *instance) {
    ...
    instance->window = SDL_CreateWindow("Raycasting example", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_SHOWN);
    if (!instance->window) {
...
void handle_input(bool *quit) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) {
            ...
            double moveSpeed = 0.08; // Adjust if necessary
            double rotSpeed = 0.05;  // Adjust if necessary
...
int main(int argc, char* argv[]) {
    ...
    // Generate a new map
    std::vector<std::vector<int> > generatedMap = generateMap(28, 28);
